...
--    for Step in 0 .. Nr_of_Steps loop
    for Step in 0 .. Nr_of_Steps/7 - 1  loop  -- TEST !!!
      ...
      -- waiting for the last process in critical to leave the critical section and reopen entry room
      -- and close exit door
      Flags(Process.Id) := 1; -- process wants to enter critical section so is in state of wainting to enter waiting room
      Condition := False;
      ...
      -- process has just entered waiting room 3
      Flags(Process.Id) := 3;
      Change_State (Entry_Protocol_2);

      for I in Flags'Range loop
      -- processes waits for other process to get through door_in(critical section)
         if Flags(I) = 1 then
            Flags(Process.Id) := 2;
            Change_State (Entry_Protocol_3);

            --waiting for the 'last' process to enter and close the door 
            Condition := False;
            while not Condition loop
               for I in Flags'Range loop
                  if Flags(I) = 4 then
      ...
      -- wait for lower processes to finish exit protocol(stand before waiting room)
      Condition := False;
      ...
      -- Ensure everyone in the waiting room has realized that door is supposed to be cloased
      Condition := False;
      while not Condition loop
         Condition := True;
