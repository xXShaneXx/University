...
-- Types, procedures and functions
  ...
  task type Node is --Interface for leave and enter, each task is like go routine, but we need to crate object first
    entry Enter; --entry - each
    entry Leave;
  end Node;

  task body Node is --how to behave
  begin
    loop
      select
        accept Enter do 
  ...
  Board : array (0 .. Board_Width-1, 0 .. Board_Height-1) of Node; --15*15 nodes each starts working by inicialization
    ...
    begin
      ...
      case N is
        when 0 =>
          Move_Up( NewPosition );
        when 1 =>
          Move_Down( NewPosition );
        when 2 =>
          Move_Left( NewPosition );
        when 3 =>
          Move_Right( NewPosition );
        when others =>
  ...
  begin
    accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      ...
      -- Looped to find a free position
      loop
        Traveler.Position := (
        ...
        select
           --calling enter by traveler
          NewPosition := Traveler.Position;Board( Traveler.Position.X, Traveler.Position.Y ).Enter;
          exit;
        else --if Node is busy
          null;
        end select;
      end loop;
