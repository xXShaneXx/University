...
-- Types, procedures and functions
  ...
  C: array (0 .. Nr_Of_Processes-1) of Integer range 0 .. 1 := (others => 1);
  ...
  task body Process_Task_Type is
    ...
    function Other_Id return Integer is
    begin
      return (Process.Id + 1) mod 2;
    end Other_Id;
...
--    for Step in 0 .. Nr_of_Steps loop
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      C(Process.Id) := 0;
      loop
        exit when C(Other_Id) /= 0;
        if Turn = Other_Id then
          C(Process.Id) := 1;
          loop exit when Turn /= Other_Id; end loop;
          C(Process.Id) := 0;
        end if;
      end loop;
