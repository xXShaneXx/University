...
#line 57 "calc.y"
                            { 
        printf("%lld ", (yyvsp[0].val)); // ONP: Liczba jest już skorygowana w Flexie
        (yyval.val) = (yyvsp[0].val);            // $$ otrzymuje wartość NUM
    }
...
#line 61 "calc.y"
                            { 
        (yyval.val) = (yyvsp[-1].val);            // Wynik w nawiasach
    }
...
#line 80 "calc.y"
        ...
        printf("^ ");
        (yyval.val) = mod_pow((yyvsp[-2].val), (yyvsp[0].val));
    }
#line 1166 "calc.tab.c"
    break;
