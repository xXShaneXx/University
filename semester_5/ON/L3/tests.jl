...
@testset "Bisection Method Tests" begin
    ...
    # 1. Exact Root: f1(2.0) = 0.0. Will stop on the epsilon check.
    ...
    # 2. Non-Exact Root: Loop stops on delta check.
    r2, v2, it2, err2 = functions.bisection_method(f2, 1.0, 2.0, delta, epsilon)
    @test abs(v2) < epsilon
    @test it2 == 17
    @test err2 == 0

    # 3. Non-Exact Root (Pi): Loop stops on delta check.
    ...
    # 4. No Sign Change (f1: [5, 10] are both positive)
    @test functions.bisection_method(f1, 5.0, 10.0, delta, epsilon) == (7.5, 52.25, 0, 1) 
    
    # 5. No Sign Change (f2: [-2, -1] are both negative)
...
@testset "Newton Method Tests" begin
    ...
    # For f(x) = x^3, df(x)=3x^2, at x=0, df=0
    f4(x) = x^3
    df4(x) = 3*x^2
    r3, v3, it3, err3 = functions.newton_method(f4, df4, 0.1, delta, epsilon, maxit)
    @test err3 == 2

    # Test case 4: Not converged in maxit
    r4, v4, it4, err4 = functions.newton_method(f2, df2, 10.0, delta, epsilon, 5)  # small maxit
    @test err4 == 1
    @test it4 == 5
end